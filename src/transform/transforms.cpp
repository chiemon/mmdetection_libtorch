...
float imrescale(const cv::Mat& image, cv::Mat& image_resize,
                const std::vector<int>& img_scale, int& net_width,
                int& net_height) {
  float h = image.rows;
  float w = image.cols;
  ...
  float scale_factor =
      std::min(max_long_edge / std::max(h, w), max_short_edge / std::min(h, w));
...
cv::Mat impad(const cv::Mat& image, std::vector<int>& pad_shape, int pad_val) {
  ...
  cv::Mat pad_image =
      cv::Mat::ones(pad_shape[0], pad_shape[1], image.type()) * pad_val;
...
void normalize(torch::Tensor& tensor_image, const std::vector<float>& mean,
...
void normalize(cv::Mat& resize_image, const std::vector<float>& mean,
               const std::vector<float>& std) {
  // std::cout <<"a3: " <<resize_image.rows <<" " <<resize_image.cols <<
  // std::endl;
  ...
  // std::cout <<"a4: " <<resize_image.rows <<" " <<resize_image.cols <<
  // std::endl;
...
void transform(const cv::Mat& image, torch::Tensor& tensor_image,
  ...
  // std::cout << int(bgr.val[0])<<" "<< int(bgr.val[1])<<" "<<
  // int(bgr.val[2])<<std::endl;
  cv::Mat image_resize;
  if (transform_params.keep_ratio_ == 0) {
    ...
    transform_params.scale_factor_ =
        imrescale(image, image_resize, transform_params.img_scale_, net_width,
                  net_height);
  }
  // std::cout << "a2: " <<image_resize.rows<<" "<<image_resize.cols<<
  // std::endl;
  ...
  // std::cout <<"a5: " <<image_resize.rows <<" " << image_resize.cols <<
  // std::endl;
  ...
  if (transform_params.pad_ > 0) {
    ...
    // std::cout <<"a6: " <<image_resize.rows <<" " << image_resize.cols <<
    // std::endl;
  ...
  tensor_image = torch::from_blob(image_resize.data,
                                  {1, image_resize.rows, image_resize.cols, 3});
