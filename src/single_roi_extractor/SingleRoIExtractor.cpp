...
void SingleRoIExtractor::init_params(
    const RoiExtractorParams& roi_extractor_params) {
...
void SingleRoIExtractor::build_roi_align_layer() {
  for (int i = 0; i < featmap_strides_.size(); i++) {
    ...
    roi_align_layers_.push_back(
        RoIAlign(out_size_, spatial_scale, sample_num_));
  }
}
torch::Tensor SingleRoIExtractor::map_roi_levels(const torch::Tensor rois,
                                                 int num_levels) {
  torch::Tensor scale =
      torch::sqrt((rois.select(1, 3) - rois.select(1, 1) + 1) *
                  (rois.select(1, 4) - rois.select(1, 2) + 1));
  torch::Tensor target_lvls =
      torch::floor(torch::log(scale / float(finest_scale_) + 1.0e-6));
  target_lvls.clamp_(0, num_levels - 1);
  return target_lvls;
}

torch::Tensor SingleRoIExtractor::bbox_roi_extractor(
    const std::vector<torch::Tensor>& feats, const torch::Tensor& rois) {
  torch::Tensor target_lvls = map_roi_levels(rois, featmap_strides_.size());
  torch::Tensor roi_feats =
      feats[0].new_zeros({rois.size(0), out_channels_, out_size_, out_size_});
  ...
  for (int i = 0; i < feats.size() - 1; i++) {
    ...
    torch::Tensor output =
        feats[i].new_zeros({num_rois, num_channels, out_size_, out_size_});
