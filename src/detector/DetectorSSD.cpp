...
DetectorSSD::DetectorSSD() { detector_type_ = DetetorType::SSD; }

DetectorSSD::~DetectorSSD() {}

void DetectorSSD::LoadParams(const Params& params,
...
void DetectorSSD::LoadTracedModule() { DetectorCommon::LoadTracedModule(); }
...
void DetectorSSD::get_min_max_size() {
  ...
  int step = int(floor(max_ratio - min_ratio) /
                 float(ssd_head_params_.anchor_ratios_.size() - 2));
  int r = min_ratio;
  while (r < max_ratio + 1) {
  ...
  if (net_width_ == 300) {
    ...
    } else if (abs(ssd_head_params_.basesize_ratio_range_[0] - 0.2) <=
               EPSILON) {
    ...
    } else if (abs(ssd_head_params_.basesize_ratio_range_[0] - 0.15) <=
               EPSILON) {
      min_sizes_.insert(std::begin(min_sizes_), int(net_width_ * 7 / 100));
      max_sizes_.insert(std::begin(max_sizes_), int(net_width_ * 15 / 100));
    } else {
...
void DetectorSSD::get_anchor_generators(
  ...
  for (int k = 0; k < anchor_head_params_.anchor_strides_.size(); k++) {
    ...
    anchor_generator.feature_maps_sizes_.push_back(
        ceil(float(net_height_) / stride));
    anchor_generator.feature_maps_sizes_.push_back(
        ceil(float(net_width_) / stride));
...
void DetectorSSD::get_anchor_boxes() {
  for (int k = 0; k < int(anchor_generators_.size()); k++) {
    std::vector<int> ind;
    for (int i = 0; i < ratios_[k].size(); i++) ind.push_back(i);
    ind.insert(ind.begin() + 1, int(ind.size()));
    torch::Tensor indices = torch::tensor(ind, torch::kLong);
    anchor_generators_[k].base_anchors_ =
        anchor_generators_[k].base_anchors_.index_select(0, indices);
...
void DetectorSSD::Detect(const cv::Mat& image,
