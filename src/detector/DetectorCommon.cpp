...
void DetectorCommon::LoadParams(const Params& params,
...
void DetectorCommon::LoadTracedModule() {
  module_ = std::make_unique<torch::jit::script::Module>(
      torch::jit::load(model_path_));
}

void DetectorCommon::get_anchor_generators(
    const std::vector<int>& anchor_base_sizes,
  ...
  for (int k = 0; k < int(base_sizes.size()); k++) {
    AnchorGenerator anchor_generator(base_sizes[k], anchor_scales,
                                     anchor_ratios);
...
void DetectorCommon::get_anchor_boxes() {
  for (int k = 0; k < int(anchor_generators_.size()); k++) {
    ...
    anchor_generators_[k].feature_maps_sizes_.push_back(
        ceil(float(net_height_) / anchor_head_params_.anchor_strides_[k]));
    anchor_generators_[k].feature_maps_sizes_.push_back(
        ceil(float(net_width_) / anchor_head_params_.anchor_strides_[k]));
    torch::Tensor all_anchors = anchor_generators_[k].grid_anchors(
        anchor_head_params_.anchor_strides_[k], *device_);
    if (k == 0) {
...
void DetectorCommon::get_proposals(torch::Tensor& output,
  ...
  for (int k = 0; k < int(anchor_generators_.size()); k++) {
    ...
    if (anchor_head_params_.nms_pre_ > 0 &&
        anchor_num > anchor_head_params_.nms_pre_) {
    ...
    torch::Tensor proposals_bboxes_layer = delta2bbox(
        anchors_layer, loc, img_shape, anchor_head_params_.target_means_,
        anchor_head_params_.target_stds_);
    if (rpn) {
      torch::Tensor inds =
          singleclass_nms(torch::cat({proposals_bboxes_layer, score_layer}, 1),
                          anchor_head_params_.nms_thresh_);
    ...
    if (k == 0) {
      ...
      proposals_bboxes =
          torch::cat({proposals_bboxes, proposals_bboxes_layer}, 0);
...
void DetectorCommon::DetectOneStage(const cv::Mat& image,
                                    std::vector<DetectedBox>& detected_boxes) {
  torch::Tensor tensor_image;
  transform(image, tensor_image, transform_params_, net_width_, net_height_,
            device_);
  ...
  torch::Tensor proposals_bboxes;
  bool rpn = false;
  get_proposals(output, transform_params_.img_shape_, proposals_bboxes,
                proposals_scores, rpn);

  if (anchor_head_params_.use_sigmoid_ == 1) {
    torch::Tensor padding = torch::zeros({proposals_scores.sizes()[0], 1})
                                .to(proposals_scores.device());
    proposals_scores = torch::cat({padding, proposals_scores}, 1);
  }

  torch::Tensor nms_results = multiclass_nms(
      proposals_bboxes, proposals_scores, anchor_head_params_.score_thresh_,
      anchor_head_params_.nms_thresh_, anchor_head_params_.max_per_img_);

  cv::Size2f scale = cv::Size2f(image.size().width / float(net_width_),
                                image.size().height / float(net_height_));
