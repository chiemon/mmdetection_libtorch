...
void DetectorRetinanet::LoadParams(const Params& params,
  ...
  for (int i = 0; i < scales_per_octave_; i++) {
    anchor_scales.push_back((pow(2, (float(i) / float(scales_per_octave_)))) *
                            octave_base_scale_);
  }
...
void DetectorRetinanet::Detect(const cv::Mat& image,
