...
void DetectorFasterRcnn::LoadParams(const Params& params,
  ...
  get_anchor_generators({}, rpn_head_params_.anchor_scales_,
                        rpn_head_params_.anchor_ratios_);
}

void DetectorFasterRcnn::LoadTracedModule() {
  bone_module_ = std::make_unique<torch::jit::script::Module>(
      torch::jit::load(bone_model_path_));
  if (with_shared_) {
    shared_module_ = std::make_unique<torch::jit::script::Module>(
        torch::jit::load(shared_model_path_));
  }
  bbox_module_ = std::make_unique<torch::jit::script::Module>(
      torch::jit::load(bbox_model_path_));
}

void DetectorFasterRcnn::get_rpn_fpn_data(
    const torch::Tensor& output, torch::Tensor& rpn_data,
  ...
  for (int k = 0; k < int(anchor_generators_.size()); k++) {
    ...
    fpn_num += anchor_generators_[k].feature_maps_sizes_[0] *
               anchor_generators_[k].feature_maps_sizes_[1] * fpn_channels;
  }
  ...
  for (int k = 0; k < int(anchor_generators_.size()); k++) {
    ...
    torch::Tensor fpn_data_layer =
        fpn_data.slice(0, start, end)
            .view({1, fpn_channels, feature_height, feature_width});
...
void DetectorFasterRcnn::Detect(const cv::Mat& image,
  ...
  torch::Tensor output =
      bone_module_->forward({tensor_image}).toTensor().squeeze(0);
  ...
  bool rpn = true;
  get_proposals(rpn_data, transform_params_.img_shape_, proposals_bboxes,
  ...
  if (rpn_head_params_.nms_across_levels_ == 1) {
    torch::Tensor inds =
        singleclass_nms(proposals, anchor_head_params_.nms_thresh_);
    int num = cv::min(int(inds.sizes()[0]), rpn_head_params_.max_num_);
    proposals = proposals.slice(0, 0, num);
  } else {
    int num =
        cv::min(rpn_head_params_.max_num_, int(proposals_scores.sizes()[0]));
    std::tuple<torch::Tensor, torch::Tensor> topk =
        torch::topk(proposals_scores.squeeze(), num);
  ...
  torch::Tensor roi_feats =
      single_roi_extractor_.bbox_roi_extractor(fpn_datas, rois);
  ...
  torch::Tensor nms_results = multiclass_nms(
      bboxes, scores, anchor_head_params_.score_thresh_,
