...
class DetectorImpl {
 public:
  ...
  virtual void LoadParams(const Params& params,
                          torch::DeviceType* device_type) = 0;
  virtual void LoadTracedModule() = 0;
  virtual void Detect(const cv::Mat& image,
                      std::vector<DetectedBox>& detected_boxes) = 0;
};
...
class DetectorCreator final {
 public:
  ...
  static std::unique_ptr<DetectorImpl> create_detector(
