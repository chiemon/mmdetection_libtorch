...
torch::Tensor delta2bbox(const torch::Tensor& rois, const torch::Tensor& deltas,
                         ...
                         const std::vector<float>& stds, float wh_ratio_clip) {
  torch::Tensor Tmeans =
      torch::tensor(means).to(deltas.device()).type_as(deltas);
  ...
  torch::Tensor px =
      ((rois.select(1, 0) + rois.select(1, 2)) * 0.5).expand_as(dx);
  torch::Tensor py =
      ((rois.select(1, 1) + rois.select(1, 3)) * 0.5).expand_as(dy);
  torch::Tensor pw =
      ((rois.select(1, 2) - rois.select(1, 0)) + 1.0).expand_as(dw);
  torch::Tensor ph =
      ((rois.select(1, 3) - rois.select(1, 1)) + 1.0).expand_as(dh);
...
torch::Tensor multiclass_nms(const torch::Tensor& multi_bboxes,
                             const torch::Tensor& multi_scores, float score_thr,
  ...
  for (int i = 1; i < num_classes; i++) {
    ...
    torch::Tensor _scores =
        multi_scores.index_select(0, cls_inds).select(1, i).unsqueeze(1);
    ...
    torch::Tensor cls_labels =
        multi_bboxes.new_full({cls_dets.sizes()[0], 1}, i - 1);
...
void bbox2result(torch::Tensor& result, float thresh, cv::Size2f scale,
  ...
  for (size_t i = 0; i < result.size(0); i++) {
    ...
    if (score > thresh) {
      ...
      detected_box.box.width =
          (result_data[i][2] - result_data[i][0]) * scale.width;
      detected_box.box.height =
          (result_data[i][3] - result_data[i][1]) * scale.height;
